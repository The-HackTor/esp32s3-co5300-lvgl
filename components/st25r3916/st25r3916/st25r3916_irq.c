...
extern struct k_sem *st25r_irq_wait_sem(void);
...
static volatile st25r3916Interrupt   st25r3916interrupt;
...
{
    struct k_sem *sem = st25r_irq_wait_sem();
    k_sem_reset(sem);
    uint32_t status = st25r3916interrupt.status & mask;
    int64_t deadline = (tmo == 0U) ? INT64_MAX : (k_uptime_get() + tmo);
    ...
    while (status == 0U) {
        ...
        if (tmo == 0U) {
            ...
            remain = K_MSEC(deadline - now);
        }
        if (k_sem_take(sem, remain) != 0) {
            break;
        }
