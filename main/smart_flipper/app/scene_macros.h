...
/*
 * X-macro contract: each app's scene_config.h is a list of
 *   ADD_SCENE(prefix, name, Id)
 * lines. The caller redefines ADD_SCENE to one of the variants below
 * (ENUM / DECL / ON_ENTER / ON_EVENT / ON_EXIT) and re-includes the
 * config to materialize that view of the scene set.
 */
