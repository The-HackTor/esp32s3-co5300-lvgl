...
static void send_cb(bool done, int progress, void *ctx)
{
    SubghzApp *app = ctx;
...
void subghz_scene_send_dump_on_enter(void *ctx)
{
    SubghzApp *app = ctx;
    last_progress = 0;

    view_action_reset(app->action);
    view_action_set_header(app->action, "Send Dump", COLOR_MAGENTA);

    NfcApp *nfc = nfc_app_get();
    if(!nfc->dump_valid) {
        view_action_set_header_detail(app->action, "No card loaded");
        view_action_set_text(app->action, LV_SYMBOL_WARNING);
        view_action_set_detail(app->action, "Read a card first", COLOR_RED);
        view_action_set_cancel_button(app->action, LV_SYMBOL_LEFT " Back",
                                      COLOR_SECONDARY, stop_cb, app);
        view_dispatcher_switch_to_view(app->view_dispatcher, SubghzViewAction);
        return;
    }

    memcpy(hw_subghz_get_relay_dump(), &nfc->dump, sizeof(nfc->dump));

    view_action_set_header_detail(app->action, "Sending...");
    ...
    hw_subghz_send_dump(send_cb, app);
}

bool subghz_scene_send_dump_on_event(void *ctx, SceneEvent event)
{
    SubghzApp *app = ctx;
    ...
    if(event.event == SUBGHZ_EVT_RELAY_PROGRESS) {
        char buf[16];
        lv_snprintf(buf, sizeof(buf), "Sending  %d%%", last_progress);
    ...
    if(event.event == SUBGHZ_EVT_RELAY_DONE) {
        ...
        view_action_set_detail(app->action, "Transfer complete", COLOR_GREEN);
...
void subghz_scene_send_dump_on_exit(void *ctx)
{
    SubghzApp *app = ctx;
