...
esp_err_t ir_remote_save(const IrRemote *in)
{
    ...
    if(fprintf(fp, "%s\n%s\n", IR_FILETYPE_SIGNALS, IR_VERSION_HEADER) < 0) {
    ...
    for(size_t i = 0; i < in->button_count; i++) {
        ...
        bool ok = fprintf(fp, "#\nname: %s\n", b->name) >= 0;
        if(ok && b->signal.type == INFRARED_SIGNAL_PARSED) {
            ok = fprintf(fp, "type: parsed\n")                          >= 0
              && fprintf(fp, "protocol: %s\n", b->signal.parsed.protocol) >= 0;
            if(ok) write_hex32_le(fp, "address", b->signal.parsed.address);
            if(ok) write_hex32_le(fp, "command", b->signal.parsed.command);
        } else if(ok) {
            ok = fprintf(fp, "type: raw\n")                                       >= 0
              && fprintf(fp, "frequency: %lu\n",
                         (unsigned long)b->signal.raw.freq_hz)                    >= 0
              && fprintf(fp, "duty_cycle: %.6f\n",
                         b->signal.raw.duty_pct / 100.0f)                         >= 0
              && fprintf(fp, "data:")                                             >= 0;
            for(size_t j = 0; ok && j < b->signal.raw.n_timings; j++) {
                ok = fprintf(fp, " %u", b->signal.raw.timings[j]) >= 0;
            }
            if(ok) ok = fprintf(fp, "\n") >= 0;
        }
        if(!ok) {
...
static IrHistoryEntry    s_hist[IR_HISTORY_MAX_ENTRIES];
static size_t            s_hist_count;
static SemaphoreHandle_t s_hist_lock;
static bool              s_hist_dirty;
static int64_t           s_hist_last_dirty_us;
static lv_timer_t       *s_hist_flush_timer;

#define HIST_FLUSH_QUIET_US     ((int64_t)2 * 1000 * 1000)
#define HIST_FLUSH_TICK_MS      1000
...
static esp_err_t hist_write_to_disk(const IrHistoryEntry *src, size_t n)
{
    if(n == 0) {
        unlink(IR_HISTORY_PATH);
        return ESP_OK;
    }
    FILE *fp = store_atomic_open(IR_HISTORY_PATH);
    if(!fp) return ESP_FAIL;
    for(size_t i = 0; i < n; i++) {
        if(fprintf(fp, "%lld %s %08lX %08lX\n",
                   (long long)src[i].timestamp_us, src[i].protocol,
                   (unsigned long)src[i].address,
                   (unsigned long)src[i].command) < 0) {
            store_atomic_abort(fp, IR_HISTORY_PATH);
            return ESP_FAIL;
        }
    }
    return store_atomic_commit(fp, IR_HISTORY_PATH);
}
...
static void hist_load_from_disk(void)
{
    ...
    char line[256];
    s_hist_count = 0;
    while(s_hist_count < IR_HISTORY_MAX_ENTRIES &&
          fgets(line, sizeof(line), fp)) {
        if(parse_history_line(line, &s_hist[s_hist_count])) s_hist_count++;
    }
    fclose(fp);
}
...
{
    if(!s_hist_lock) return ESP_OK;

    IrHistoryEntry *snap = malloc(sizeof(s_hist));
    if(!snap) return ESP_ERR_NO_MEM;

    xSemaphoreTake(s_hist_lock, portMAX_DELAY);
    if(!s_hist_dirty) { xSemaphoreGive(s_hist_lock); free(snap); return ESP_OK; }
    size_t n = s_hist_count;
    if(n) memcpy(snap, s_hist, n * sizeof(IrHistoryEntry));
    s_hist_dirty = false;
    xSemaphoreGive(s_hist_lock);

    esp_err_t err = hist_write_to_disk(snap, n);
    free(snap);
...
static void hist_flush_tick(lv_timer_t *t)
{
    (void)t;
    if(!s_hist_dirty) return;
    if(esp_timer_get_time() - s_hist_last_dirty_us < HIST_FLUSH_QUIET_US) return;
    ir_history_flush();
}
...
static void hist_init(void)
{
    ...
    hist_load_from_disk();
    s_hist_flush_timer = lv_timer_create(hist_flush_tick, HIST_FLUSH_TICK_MS, NULL);
}

esp_err_t ir_history_append(const IrHistoryEntry *e)
{
    if(!e) return ESP_ERR_INVALID_ARG;
    if(!s_hist_lock) return ESP_ERR_INVALID_STATE;

    xSemaphoreTake(s_hist_lock, portMAX_DELAY);
    if(s_hist_count == IR_HISTORY_MAX_ENTRIES) {
        memmove(&s_hist[0], &s_hist[1],
                (IR_HISTORY_MAX_ENTRIES - 1) * sizeof(IrHistoryEntry));
        s_hist_count--;
    }
    s_hist[s_hist_count++]   = *e;
    s_hist_dirty             = true;
    s_hist_last_dirty_us     = esp_timer_get_time();
    xSemaphoreGive(s_hist_lock);
    return ESP_OK;
}
...
esp_err_t ir_history_read(IrHistoryEntry *out, size_t cap, size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    xSemaphoreTake(s_hist_lock, portMAX_DELAY);
    size_t to_copy = s_hist_count < cap ? s_hist_count : cap;
    for(size_t i = 0; i < to_copy; i++) {
        out[i] = s_hist[s_hist_count - 1 - i];
    }
    *out_count = to_copy;
    xSemaphoreGive(s_hist_lock);
    return ESP_OK;
...
esp_err_t ir_history_delete(size_t idx)
{
    ...
    xSemaphoreTake(s_hist_lock, portMAX_DELAY);
    if(idx >= s_hist_count) { xSemaphoreGive(s_hist_lock); return ESP_ERR_INVALID_ARG; }
    size_t file_idx = s_hist_count - 1 - idx;
    if(file_idx < s_hist_count - 1) {
        memmove(&s_hist[file_idx], &s_hist[file_idx + 1],
                (s_hist_count - file_idx - 1) * sizeof(IrHistoryEntry));
    }
    s_hist_count--;
    s_hist_dirty         = true;
    s_hist_last_dirty_us = esp_timer_get_time();
...
esp_err_t ir_history_clear(void)
{
    ...
    if(unlink(IR_HISTORY_PATH) != 0 && errno != ENOENT) return ESP_FAIL;
    return ESP_OK;
}
                            ...
                            uint32_t address, uint32_t command)
{
    ...
    FILE *fp = store_atomic_open(IR_RECENTS_PATH);
    if(!fp) return ESP_FAIL;
    for(size_t i = 0; i < n; i++) {
        if(fprintf(fp, "%s %s %08lX %08lX\n",
                   rows[i].label, rows[i].protocol,
                   (unsigned long)rows[i].address,
                   (unsigned long)rows[i].command) < 0) {
            store_atomic_abort(fp, IR_RECENTS_PATH);
            return ESP_FAIL;
        }
    }
    return store_atomic_commit(fp, IR_RECENTS_PATH);
}
