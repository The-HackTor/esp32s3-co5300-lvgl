                         ...
                         ir_decode_result_t *out)
{
    ...
    out->id = IR_CODEC_CORONA_AC;

    size_t cursor = 0;
    uint16_t pos = 0;
    for(uint8_t section = 0; section < IR_CORONA_AC_SECTIONS; section++) {
        if(!decode_byte_section(
            timings, n_timings, &cursor,
            IR_CORONA_AC_HDR_MARK, IR_CORONA_AC_HDR_SPACE,
            IR_CORONA_AC_BIT_MARK, IR_CORONA_AC_ONE_SPACE,
            IR_CORONA_AC_BIT_MARK, IR_CORONA_AC_ZERO_SPACE,
            IR_CORONA_AC_BIT_MARK, IR_CORONA_AC_SPACE_GAP,
            true,
            IR_DEFAULT_TOLERANCE + 5, IR_MARK_EXCESS,
            out->state + pos, IR_CORONA_AC_SECTION_BYTES)) {
            if(section == 0) return false;
            break;
        }
        pos += IR_CORONA_AC_SECTION_BYTES;
        if(cursor >= n_timings) break;
    }
    if(pos * 8 != IR_CORONA_AC_BITS && pos * 8 != IR_CORONA_AC_BITS_SHORT)
        return false;

    out->bits         = pos * 8;
    out->state_nbytes = pos;
    return true;
}
